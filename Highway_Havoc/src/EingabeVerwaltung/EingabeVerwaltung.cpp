...
void EingabeVerwaltung::gruppeHinzufuegen()
{
	this->gruppenStatus.push_back(new std::vector<bool*>);
	this->gruppenStatusGeaendert.push_back(new std::vector<bool*>);
}

void EingabeVerwaltung::tasteZuGruppeHinzufuegen(short index, short indexInTasten)
{
	std::list<bool>::iterator tastenStatusIterator;
	tastenStatusIterator = this->tastenStatus.begin();
	advance(tastenStatusIterator, indexInTasten);
	this->gruppenStatus[index]->push_back(&*tastenStatusIterator);

	std::list<bool>::iterator tastenStatusIteratorGeaendert;
	tastenStatusIteratorGeaendert = this->tastenStatusGeaendert.begin();
	advance(tastenStatusIteratorGeaendert, indexInTasten);
	this->gruppenStatusGeaendert[index]->push_back(&*tastenStatusIteratorGeaendert);
}

bool EingabeVerwaltung::getGruppenStatus(short index)
{
	bool istGedrueckt = false;
	for (int i = 0; i < this->gruppenStatus[index]->size(); i++) {
		if (*(*this->gruppenStatus[index])[i]) istGedrueckt = true;
	}
	return istGedrueckt;
}

bool EingabeVerwaltung::getGruppenStatusGeaendert(short index)
{
	bool istGedrueckt = false;
	for (int i = 0; i < this->gruppenStatusGeaendert[index]->size(); i++) {
		if (*(*this->gruppenStatusGeaendert[index])[i]) istGedrueckt = true;
	}
	return istGedrueckt;
}
